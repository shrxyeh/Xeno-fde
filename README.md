...
1. **tenants** - Shopify merchants (root entity for multi-tenancy)
2. **users** - Dashboard users (ADMIN/VIEWER roles)
3. **customers** - Shopify customers synced from stores
4. **products** - Product catalog
5. **orders** - Order history with financial/fulfillment status
6. **order_items** - Line items for each order
7. **events** - Engagement events (cart abandoned, checkout started, etc.)

### Relationships

- Tenant → Users, Customers, Products, Orders, Events (1:N)
- Customer → Orders (1:N)
- Order → OrderItems (1:N)
- Product ↔ OrderItems (N:M)

Every table (except tenants) includes `tenantId` for data isolation. Full schema available in `backend/prisma/schema.prisma`.
...
4. Configure service:
   ...
   - Start: `npm start`
5. Add environment variables (see Quick Start section for list)

**Frontend:**
1. Create account at https://vercel.com
2. Import repository
3. Framework: Next.js, Root: `frontend`
4. Add env: `NEXT_PUBLIC_API_URL=<railway-backend-url>`

### Render Alternative

1. Create PostgreSQL database on Render
2. Create Web Service, connect repo
3. Root: `backend`, Build: `npm install && npx prisma migrate deploy`
4. Deploy frontend to Vercel (same as above)

### Post-Deployment

- Update Shopify app URLs in Partner Dashboard
- Update `SHOPIFY_APP_URL` and `FRONTEND_URL` in backend env vars
